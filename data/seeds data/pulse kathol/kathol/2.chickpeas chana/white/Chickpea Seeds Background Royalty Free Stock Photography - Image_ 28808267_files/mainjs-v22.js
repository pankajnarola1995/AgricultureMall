...
// Image mouseover handler
function SearchSubmitAjaxOnMouseOver() {
  try {
    imageZLoaders();
  } catch(e) {};
  var r = $(this).data('r');
  var f = r[7] == 65535 ? trailOn4 : trailOn3;
  f(r[8], r[9], r[10], r[0], r[11], r[12], r[13], r[14], r[15], r[16], r[4]);
}

// Image mouseout handler
function SearchSubmitAjaxOnMouseOut() {
  var r = $(this).data('r');
  if (r[7] == 65535) {
...
function SearchSubmitAjaxHandler(result, action, skipState) {
  // Load page normally, if needed
  if (!result || !result.images || !result.images.length ||
      result.images.length && !$(".thb-large-box").not("#images_header .thb-large-box").length ||
      !window.history.pushState || result.href === true) {
    location.href = action || location.href;
    return false;
  }
  if (result.href) {
  ...
  // Set meta data and history state
  if (!skipState) {
    document.title = result.title;
    $("meta[name='description']").attr({content: result.meta_description});
    $("meta[name='keywords']").attr({content: result.meta_keywords});
    try {
      if (location.hash.match(/^#details(\d+)$/) && skipState !== true) {
        window.statepushed = true;
        window.history.pushState({type: 'pagination', srh_field: $("#srh_field").val(), href: location.href.replace(/#.*$/, "")}, window.title, location.href.replace(/#.*$/, ""));
      }
      window.statepushed = true;
      window.history.pushState({type: 'pagination', srh_field: $("#srh_field").val(), href: action}, result.title, action);
    } catch (e) {console.log(e);}
  }
  // Replace header and footer
  if (result.images_header) {
    $("#images_header").html(result.images_header);
  }
  if (result.images_footer) {
    $("#images_footer").html(result.images_footer);
  }
  // Set navigation handlers
  SlideHandleKeyPress._keynaviprev = result.prevlink;
  SlideHandleKeyPress._keynavinext = result.nextlink;
  // Set next button handlers
  var $n = $(".dt-next a");
  if ($n.length) {
    $("#nextbutton a").attr({
      title: $n.attr('title'),
  ...
  $(".thb-large-box").stop().fadeOut(500, function() {
    ...
    for (var i = 0; i < result.images.length; i++) {
      ...
      $b.attr({
        ...
        visibility: 'hidden'
      }).mouseover(SearchSubmitAjaxOnMouseOver)
      .mouseout(SearchSubmitAjaxOnMouseOut)
      .parent().attr({href: r[3]})
      .click(SearchSubmitAjaxToggleImageDetails);
    }
    SearchSubmitAjaxLoadImage.loaders = {};
    imageLoaders.imgLoaders = {};
    delete imageZLoaders.imgLoaders;
    imageLoaders();
    // Set lightbox handlers
    if (typeof lightbox_updateThumbButtons != 'undefined') {
    ...
    $(".thb-large-box").stop().fadeIn(1000, function() {
      ...
      var scrollDelta = ($("html").scrollTop() || $("body").scrollTop()) - scrollTop;
      if (scrollDelta > 0) {
        SearchSubmitAjaxHandler.sts = new Date().getTime();
        $("html, body").animate({
          scrollTop: scrollTop
        }, {
          // Constant speed, approx 3000px/s
          duration: scrollDelta / 3,
          step: function() {
            if ($.fx.off) {
              return;
            }
            if (new Date().getTime() - SearchSubmitAjaxHandler.sts > 1500) {
              // Slow computer, disable animation
              $.fx.off = true;
...
// Cache ajax results
function SearchSubmitCacheResult(action, result) {
  SearchSubmitAjax.results[action] = result;
  // Keep last 5 pages in cache
...
// Ajax search handler (for search form or links)
function SearchSubmitAjax(action, skipState) {
  ...
  $("#sampleMovie").trigger('pause');
  trailOff2();
  SearchSubmitAjaxCloseImageDetails(true);
  ...
  SearchSubmitAjax.results[action] = $.ajax({
    type: "POST",
    url: action,
    data: {images_ajax: 1},
    success: function(result) {
      SearchSubmitCacheResult(action, result);
...
function SearchSubmitAjaxCheck() {
  if ($("#s_st").val() != "wre") {
    // Searching from a page different than search.php
    return SearchSubmit();
  }
...
// Preload next page of search results
  ...
  $(".dt-next a").each(function() {
    ...
    if (action) {
      ...
      if (typeof SearchSubmitAjax.results[action] == "undefined") {
        SearchSubmitAjax.results[action] = $.post(action, {images_ajax: 1, securitycheck: secPin}, function(result) {
          SearchSubmitCacheResult(action, result);
...
// Keyboard navigation between inline item details
function SearchSubmitAjaxImageDetailsNav(e) {
  e = e || {};
  var key = e.which || e.keyCode;
  var crt_cell = $(".id_crt_img");
  if (crt_cell.length == 0) {
    SlideHandleKeyPress(e);
    return false;
  }
  if (key == 37) {
    $(".grid-system-prev", "#grid-system-expander").click();
    return false;
  }
  if (key == 39) {
    $(".grid-system-next", "#grid-system-expander").click();
    return false;
  }
  if (key == 27) {
    $(".grid-system-close", "#grid-system-expander").click();
    return false;
  }
  return true;
}

function SearchSubmitAjaxAlignCols() {
  var hmin = 405;
  var h1 = $(".grid-col-1", "#grid-system-expander").height();
  var h2 = $(".grid-col-2", "#grid-system-expander").height();
  $(".grid-col-1", "#grid-system-expander").css({
    marginTop: Math.max(0, Math.round((Math.max(hmin, h2) - h1) / 2))
  });
  if ($(".grid-col-2 .dt-photo-info", "#grid-system-expander").length) {
    $(".grid-col-2", "#grid-system-expander").css({
      marginTop: Math.max(0, Math.round((Math.max(hmin, h1) - h2) / 2))
    });
  }
}

function SearchSubmitAjaxWrapImage() {
  var crt_cell = $(".id_crt_img");
  if (crt_cell.length == 0) {
    return;
  }
  var id = crt_cell.attr("id").replace("thb_cell", "");
  var type = SearchSubmitAjaxLoadImage.loaders.info[id][0] || "t";
  var width = SearchSubmitAjaxLoadImage.loaders.info[id][1] || "";
  var height = SearchSubmitAjaxLoadImage.loaders.info[id][2] || "";
  var src = SearchSubmitAjaxLoadImage.loaders[type + 'src'][id];
  if (!src && type != "t") {
    type = (SearchSubmitAjaxLoadImage.loaders['tsrc'][id].indexOf("/x/") != -1) ? "x" : "t";
    src = SearchSubmitAjaxLoadImage.loaders['tsrc'][id];
  }
  if (width >= height) {
    $(".dt-photo", "#grid-system-expander").css({
      width: width,
      height: ''
    });
  } else {
  ...
  $(".dt-photo", "#grid-system-expander").attr('src', src);
  if (type == 'z') {
    if (width >= height) {
      $(".dt-photo-wrap", "#grid-system-expander").css({
        width: '',
        height: $(".dt-photo", "#grid-system-expander").height() - Math.ceil($(".dt-photo", "#grid-system-expander").width()*90/1300) + 'px',
        paddingLeft: ($(".grid-col-1", "#grid-system-expander").width() - width) / 2 - 30 + 'px',
        marginBottom: '8px'
      });
    } else {
      $(".dt-photo-wrap", "#grid-system-expander").css({
        width: $(".dt-photo", "#grid-system-expander").width() - Math.ceil($(".dt-photo", "#grid-system-expander").height()*90/1300) + 'px',
        height: '',
        paddingLeft: ($(".grid-col-1", "#grid-system-expander").width() - width) / 2 - 30 + 'px',
        marginBottom: ''
      });
...
function SearchSubmitAjaxLoadImage(cell) {

  if (cell.length == 0 || cell.attr('id') == 'nextbutton') {
    return;
  }

  var width, height, type, cwidth;
  if ($(".thb_c23", cell).hasClass("thb_c23_vid")) {
    cwidth = "";
    width = 580;
    height = 326;
    type = 't';
  } else {
    var maxw = Math.max(300, $(".grid-col-1", "#grid-system-expander").width() - 60);
    var maxh = Math.max(400, $(window).height() - 240, $(".grid-col-2", "#grid-system-expander").height());
    if (typeof useZThumbs == "undefined") {
      maxw = Math.min(400, maxw);
      maxh = Math.min(450, maxh);
    }
    var iw = $("img", cell).data('r')[1];
    var ih = $("img", cell).data('r')[2];
    if (iw > ih) {
      width = maxw;
      height = width * ih / iw;
      if (height > maxh) {
  ...
  var id = cell.attr("id").replace("thb_cell", "");

  SearchSubmitAjaxLoadImage.loaders = SearchSubmitAjaxLoadImage.loaders || {};
  SearchSubmitAjaxLoadImage.loaders.info = SearchSubmitAjaxLoadImage.loaders.info || {};
  SearchSubmitAjaxLoadImage.loaders.info[id] = [type, width, height, cwidth];

  SearchSubmitAjaxLoadImage.loaders['tsrc'] = SearchSubmitAjaxLoadImage.loaders['tsrc'] || {};
  SearchSubmitAjaxLoadImage.loaders['tsrc'][id] = $("img", cell).attr('src');

  if (type == 'x' || type == 'z') {
    SearchSubmitAjaxLoadImage.loaders[type + 'src'] = SearchSubmitAjaxLoadImage.loaders[type + 'src'] || {};
    if (SearchSubmitAjaxLoadImage.loaders['tsrc'][id].indexOf("/" + type + "/") != -1) {
      SearchSubmitAjaxLoadImage.loaders[type + 'src'][id] = SearchSubmitAjaxLoadImage.loaders[type + 'src'][id];
    } else if (typeof SearchSubmitAjaxLoadImage.loaders[type + 'src'][id] == 'undefined') {
      SearchSubmitAjaxLoadImage.loaders[type + 'src'][id] = "";
      SearchSubmitAjaxLoadImage.loaders[type + 'img'] = SearchSubmitAjaxLoadImage.loaders[type + 'img'] || {};
      SearchSubmitAjaxLoadImage.loaders[type + 'img'][id] = new Image();
      SearchSubmitAjaxLoadImage.loaders[type + 'img'][id].onload = function() {
        SearchSubmitAjaxLoadImage.loaders[type + 'src'][id] = this.src;
        if (cell.get(0) == $(".id_crt_img").get(0)) {
          SearchSubmitAjaxWrapImage();
        }
      }
      SearchSubmitAjaxLoadImage.loaders[type + 'img'][id].src = SearchSubmitAjaxLoadImage.loaders['tsrc'][id].replace("static.", "thumbs.").replace("/t/", "/" + type + "/").replace("/x/", "/" + type + "/");
    }
  }

}

function SearchSubmitAjaxShowPanel() {
  var crt_cell = $(".id_crt_img");

  SearchSubmitAjaxLoadImage(crt_cell, true);
  // Preload next image
  if (SearchSubmitAjaxShowPanel.direction === 'prev') {
    SearchSubmitAjaxLoadImage(crt_cell.prevAll(".thb_cell").eq(0));
  } else {
    SearchSubmitAjaxLoadImage(crt_cell.nextAll(".thb_cell").eq(0));
  }
  SearchSubmitAjaxWrapImage();
  SearchSubmitAjaxAlignCols();

  $(".dt-photo-name a", "#grid-system-expander").attr('href', $("a", crt_cell).attr('href')).html($("img", crt_cell).data('r')[6]);
  $(".dt-photo-wrap a", "#grid-system-expander").attr('href', $("a", crt_cell).attr('href')).unbind('click');
  if ($(".thb_c23", crt_cell).hasClass("thb_c23_vid")) {
    $(".dt-photo-wrap a", "#grid-system-expander").bind('click', function() {return false;});
  }
  var left = crt_cell.position().left;
  var width = crt_cell.width();
  $(".grid-system-current", "#grid-system-expander").css({
    left: left + width / 2 - 13
  }).show();
  $("#grid-system-expander").css({
    overflow: '',
    height: ''
  });
  SearchSubmitAjaxImageScrollView();
}

function SearchSubmitAjaxImageScrollView() {
  var crt_cell = $(".id_crt_img");
  var ct = crt_cell.offset().top;
...
function SearchSubmitAjaxOpenPanel(animate) {
  var crt_cell = $(".id_crt_img");
  var left = crt_cell.position().left;
  var width = crt_cell.width();

  if (crt_cell.get(0) != SearchSubmitAjaxOpenImageDetails.old_crt_cell.get(0)) {
    SearchSubmitAjaxLoadImage(crt_cell);
    SearchSubmitAjaxWrapImage();
    SearchSubmitAjaxAlignCols();
    if (typeof SearchSubmitAjaxOpenPanel.expander == "undefined") {
      SearchSubmitAjaxOpenPanel.expander = $(".grid-system-expander").remove();
    }
    $("#grid-system-expander").remove();
    SearchSubmitAjaxOpenPanel.expander.clone().attr({id: 'grid-system-expander'}).show().insertBefore(".id_margin_img");
  } else {
    $("#grid-system-expander").insertBefore(".id_margin_img");
  }
  $(".grid-system-current", "#grid-system-expander").hide().css({
    left: left + width / 2 - 13
  });
  if (animate) {
    $("#grid-system-expander").css({
      ...
      height: 450
    }, SearchSubmitAjaxShowPanel);
  } else {
    SearchSubmitAjaxShowPanel();
  }
}

function SearchSubmitAjaxCloseImageDetails(skipState) {
    $(document).unbind('keydown', SearchSubmitAjaxImageDetailsNav);
    InitSlideKeys();

    if (location.hash.match(/^#details(\d+)$/) && skipState !== true) {
      try {
        window.statepushed = true;
        window.history.pushState({type: 'details', srh_field: $("#srh_field").val()}, window.title, location.href.replace(/#.*$/, ""));
      } catch (e) {}
    }
    $("img", ".id_crt_img").css({borderColor: ""});
    $(".id_crt_row").removeClass("id_crt_row id_crt_img").stop().animate({opacity: 1});
    $(".id_margin_img").removeClass("id_margin_img");
    $("#grid-system-expander").css({
      overflow: 'hidden'
    }).slideUp();
}

// Set add to/remove from lightbox handlers
function SearchSubmitAjaxSetAddToLightbox(crt_cell) {
  if (!crt_cell.length) {
    return;
  }
  var id = crt_cell.attr("id").replace("thb_cell", "");
  if ($("#class_thb_c23" + id).hasClass("thb_c23_rmv")) {
    $("#lightboxbutton", "#grid-system-expander").removeClass("add-lightbox").addClass("remove-lightbox").unbind('click').click(function() {
      lightbox_deleteImage(id);
      $("#class_thb_c23" + id).removeClass("thb_c23_rmv");
      SearchSubmitAjaxSetAddToLightbox($(".id_crt_img"));
      return false;
    }).attr('title', $("#class_thb_c23" + id).hasClass("thb_c23_vid") ? lightbox_vars.remove_video_from_lightbox : lightbox_vars.remove_image_from_lightbox);
  } else {
    $("#lightboxbutton", "#grid-system-expander").removeClass("remove-lightbox").addClass("add-lightbox").unbind('click').click(function() {
      lightbox_addImage(id);
      $("#class_thb_c23" + id).addClass("thb_c23_rmv");
      SearchSubmitAjaxSetAddToLightbox($(".id_crt_img"));
      return false;
    }).attr('title', $("#class_thb_c23" + id).hasClass("thb_c23_vid") ? lightbox_vars.add_video_to_lightbox : lightbox_vars.add_image_to_lightbox);
  }
}

function SearchSubmitAjaxPrefillImageDetails(crt_cell) {
  var id = crt_cell.attr("id").replace("thb_cell", "");
  $(".grid-col-2", "#grid-system-expander").html(SearchSubmitAjaxLoadImageDetails.details[id]);
  SearchSubmitAjaxSetAddToLightbox(crt_cell);
  SearchSubmitAjaxAlignCols();
  // Preload next details
  var cell = (SearchSubmitAjaxShowPanel.direction === 'prev') ? crt_cell.prevAll(".thb_cell").eq(0) : crt_cell.nextAll(".thb_cell").eq(0);
  if (cell.length) {
    id = cell.attr("id").replace("thb_cell", "");
    if (typeof SearchSubmitAjaxLoadImageDetails.details[id] == "undefined") {
      SearchSubmitAjaxLoadImageDetails(cell);
    }
  }
}

function SearchSubmitAjaxLoadImageDetails(cell, compute) {
  if (cell.length == 0 || cell.attr('id') == 'nextbutton') {
    return;
  }
  var id = cell.attr("id").replace("thb_cell", "");
  if (SearchSubmitAjaxLoadImageDetails.t) {
    clearTimeout(SearchSubmitAjaxLoadImageDetails.t);
  }
  if (SearchSubmitAjaxLoadImageDetails.t2) {
    clearTimeout(SearchSubmitAjaxLoadImageDetails.t2);
  }
  SearchSubmitAjaxLoadImageDetails.details = SearchSubmitAjaxLoadImageDetails.details || {};
  var id = cell.attr("id").replace("thb_cell", "");
  if (SearchSubmitAjaxLoadImageDetails.details[id] && !compute && cell.get(0) == $(".id_crt_img").get(0)) {
    SearchSubmitAjaxPrefillImageDetails(cell);
    return;
  }
  SearchSubmitAjaxLoadImageDetails.t = setTimeout(function() {
    $.get("/ajax/item_sizestablelog-inline.php?imageid=" + id + (compute ? "&compute=1" : ""), function(data) {
      SearchSubmitAjaxLoadImageDetails.details[id] = data;
      // Keep last 7 details in cache
      SearchSubmitAjaxLoadImageDetails.cache = SearchSubmitAjaxLoadImageDetails.cache || [];
      SearchSubmitAjaxLoadImageDetails.cache.push(id);
      if (SearchSubmitAjaxLoadImageDetails.cache.length > 7) {
        var a = SearchSubmitAjaxLoadImageDetails.cache.splice(0, 1);
        delete SearchSubmitAjaxLoadImageDetails.details[a];
      }
      if (cell.get(0) == $(".id_crt_img").get(0)) {
        SearchSubmitAjaxPrefillImageDetails(cell);
      }
  ...
  if (compute) {
    SearchSubmitAjaxLoadImageDetails.t2 = setTimeout(function() {
      SearchSubmitAjaxLoadImageDetails(cell, compute);
    }, 10000);
  }
}

function SearchSubmitAjaxOpenImageDetails(crt_cell, skipState) {
  if (crt_cell.length == 0 || crt_cell.attr('id') == 'nextbutton') {
    return;
  }

  $(document).unbind('keydown', SearchSubmitAjaxImageDetailsNav).bind('keydown', SearchSubmitAjaxImageDetailsNav);
  var event = 'keydown';
  var handler = SlideHandleKeyPress;
  if (document.addEventListener) document.removeEventListener(event, handler, false);
  else document.detachEvent('on' + event, handler);

  $("img", ".id_crt_img").css({borderColor: ""});
  SearchSubmitAjaxOpenImageDetails.old_crt_cell = $(".id_crt_img").removeClass("id_crt_img");
  SearchSubmitAjaxOpenImageDetails.old_margin_cell = $(".id_margin_img").removeClass("id_margin_img");
  $(".id_crt_row").removeClass("id_crt_row").stop().animate({opacity: 1});
  crt_cell.addClass("id_crt_row id_crt_img");
  var row = crt_cell.data('row');
  crt_cell.prevAll(".thb_cell").each(function() {
    if ($(this).data('row') !== row) {
      return false;
    }
    $(this).addClass("id_crt_row");
  });
  crt_cell.nextAll(".thb_cell").each(function() {
    if ($(this).data('row') != row) {
      $(this).addClass("id_margin_img");
      return false;
    }
    $(this).addClass("id_crt_row");
  });
  $("img", ".id_crt_img").css({borderColor: "#999794"});
  $(".id_crt_row").not(".id_crt_img").stop().animate({opacity: 0.6});
  if ($(".id_margin_img").length == 0) {
    if ($("#id_margin_img").length == 0) {
      $(".thb-large-box").append('<div id="id_margin_img" class="id_margin_img" />');
    } else {
      $("#id_margin_img").addClass("id_margin_img");
    }
  }
  if (SearchSubmitAjaxOpenImageDetails.old_crt_cell.prevAll('.thb_cell').filter(crt_cell).length > 0) {
    SearchSubmitAjaxShowPanel.direction = 'prev';
  } else {
    SearchSubmitAjaxShowPanel.direction = 'next';
  }
  if ($(".id_margin_img").get(0) != SearchSubmitAjaxOpenImageDetails.old_margin_cell.get(0)) {
    SearchSubmitAjaxOpenPanel(SearchSubmitAjaxOpenImageDetails.old_margin_cell.length == 0);
  } else {
    if ($(".id_crt_img").get(0) != SearchSubmitAjaxOpenImageDetails.old_crt_cell.get(0)) {
      $("#grid-system-expander").replaceWith(SearchSubmitAjaxOpenPanel.expander.clone().attr({id: 'grid-system-expander'}).show());
    }
    SearchSubmitAjaxShowPanel();
  }
  var id = crt_cell.attr("id").replace("thb_cell", "");
  if ($(".id_crt_img").get(0) != SearchSubmitAjaxOpenImageDetails.old_crt_cell.get(0) && skipState !== true) {
    try {
      window.statepushed = true;
      window.history.pushState({type: 'details', srh_field: $("#srh_field").val(), id: id}, window.title, "#details" + id);
    } catch(e) {}
  }
  $("#sampleMovie").trigger('pause');
  trailOff2();
  $(".grid-system-prev", "#grid-system-expander").unbind('click').click(function() {
    SearchSubmitAjaxShowPanel.direction = 'prev';
    SearchSubmitAjaxOpenImageDetails(crt_cell.prevAll('.thb_cell').eq(0));
  });
  $(".grid-system-next", "#grid-system-expander").unbind('click').click(function() {
    SearchSubmitAjaxShowPanel.direction = 'next';
    SearchSubmitAjaxOpenImageDetails(crt_cell.nextAll('.thb_cell').eq(0));
  });
  $(".grid-system-close", "#grid-system-expander").unbind('click').click(SearchSubmitAjaxCloseImageDetails);
  if ($(".id_crt_img").get(0) != SearchSubmitAjaxOpenImageDetails.old_crt_cell.get(0)) {
    SearchSubmitAjaxLoadImageDetails(crt_cell);
  }
}

function SearchSubmitAjaxToggleImageDetails(event) {
  if (event && $(event.target).is(".thb_c23")) {
    return false;
  }
  var crt_cell = $(this).closest(".thb_cell");
  if (crt_cell.hasClass("id_crt_img")) {
    SearchSubmitAjaxCloseImageDetails();
  } else {
    SearchSubmitAjaxOpenImageDetails(crt_cell);
  }
  return false;
}
...
function SearchPageInit() {
  ...
  // Set inline image details handler
  $(".thb_cell a", ".thb-large-box").click(SearchSubmitAjaxToggleImageDetails);
  // Set browser navigation handler
  window.onpopstate = function(e) {
    if (!window.statepushed) {
      // Per Chromium issue 63040, Chrome dispatches a popState event on initial page load
      return;
    }
    // Restore search field
    if (e && e.state && e.state.srh_field) {
      $("#srh_field").val(e.state.srh_field);
    }
    // Open or close inline image details
    if (e && e.state && e.state.type === 'details') {
      var id = e.state.id;
      if (id) {
        SearchSubmitAjaxOpenImageDetails($("#thb_cell" + id), true);
      } else {
        SearchSubmitAjaxCloseImageDetails(true);
      }
      return;
    ...
    if (e && e.state && e.state.type === 'pagination' && e.state.href) {
      SearchSubmitAjax(e.state.href, true);
    } else {
  ...
  // Preload next page
  SearchSubmitAjaxPreloadNext();
  // Set inline image details reposition handler
  arrangeImages.callback = function() {
    SearchSubmitAjaxOpenImageDetails($(".id_crt_img"));
  };
  // Open inline image details
  if (location.hash.match(/^#details(\d+)$/)) {
    var id = RegExp.$1;
    SearchSubmitAjaxOpenImageDetails($("#thb_cell" + id));
    window.statepushed = false;
  }
}
...
if(typeof e != "undefined")
	...
	if (right != $("#btcontainer").data('right'))
		{
		$("#btcontainer").data('right', right);
		var imgid = $("#btcontainer").data('imgid');
		if (imgid)
			{
			if (right)
				$("#class_thb_c23" + imgid).addClass('thb_c23_right');
			else
				$("#class_thb_c23" + imgid).removeClass('thb_c23_right');
			}
		}
	}
