...
function trailOn4(thumbimg,imgtitle,username,imgid,imgsize,credit,level,thw,thh,smallmedium,smallThumbPath){
	if ($("#grid-system-expander").is(":visible")) {
		return;
	}
	var smallmedium = (smallmedium == null) ? 0 : smallmedium;
	var isinstore = 0;
	...
	gettrailobj().visibility="visible";
	$("#imgtrailer").unbind().click(function() {
		this.href = $("#bigthumb" + imgid).parents("a").eq(0).attr("href");
	});
	divw = parseInt(thw)+25;
	divh = parseInt(thh)+130;
	mm_hoverThumb=1;
...
function trailOn3(thumbimg, imgtitle, username, imgid, imgsize, credit, level, thw, thh, smallmedium, smallThumbPath) {
  ...
  thumbimgprev=$("#bigthumb" + imgid).data('loadedZ')?thumbimg:smallThumbPath;
  gettrailobj().left="-500px";
  ...
  gettrailobject().style.zIndex=99;
  $("#btcontainer").data('imgid', imgid);
  if ($("#bigthumb" + imgid).data('loadedZ')) {
    $("#btcontainer").attr('src', thumbimg);
  } else {
...
function setImportantStyle(el, style) {
  try {
    ...
    // IE8
    if (!el.id) {
      $(el).attr({id: "el" + new Date().getTime()});
    }
    var ie8is = $(document).data('ie8is') || {};
    ie8is[el.id] = ie8is[el.id] || {};
    for (var property in style) {
      var value = style[property];
      ie8is[el.id][property] = value;
    }
    $(document).data('ie8is', ie8is);
    $("#ie8is").remove();
    var s = '<style id="ie8is">';
    for (var elid in ie8is) {
      s += '#' + elid + ' {';
      for (var property in ie8is[elid]) {
        var value = ie8is[elid][property];
  ...
  for (var i = 0; i < imgSizes.length; i++) {
    ...
    if (drawRow) {
      ...
      for (var j = 0; j < row.length; j++) {
        ...
        $(sel + "bigthumb" + rid, $container).data('arranged', true);
        if ($(sel + "bigthumb" + rid, $container).data('loaded')) {
          $(".showonload", sel + "thb_cell" + rid, $container).css({'visibility': 'visible'});
          if ($(sel + "bigthumb" + rid, $container).data('lastimg')) {
        ...
        rw += p;
        if (rid == "nextbutton") {
          var $el = $(sel + rid, $container);
          var el = $el.get(0);
          if (el) {
            $el.data({row: rows});
            setImportantStyle(el, {
              'margin-bottom': Math.round((maxHeight - imgSizes[rij][2]) / 2) + 'px',
              'padding-left': p + 'px'
            });
          }
        } else {
          var $el = $(sel + "thb_cell" + rid, $container);
          var el = $el.get(0);
          if (el) {
            $el.data({row: rows});
            if (dpmr > 0 && rows >= dpmr) {
              ...
              setImportantStyle(el, {
                'width': Math.floor(riw + dtx) + 'px',
                'height': Math.round(maxHeight + dty + dy) + 'px',
                'padding-left': p + 'px'
              });
            }
          }
...
function imageZLoaders() {
  if (typeof imageZLoaders.imgLoaders != "undefined") {
    return;
  }
  imageZLoaders.imgLoaders = imageZLoaders.imgLoaders || {};
  var id;
  $('.thb_cell').each(function() {
    ...
    imageZLoaders.imgLoaders[id] = new Image();
    imageZLoaders.imgLoaders[id].onload = (function(id) {
      return function() {
        $("#bigthumb" + id).data('loadedZ', true);
      };
    })(id);
    imageZLoaders.imgLoaders[id].src = $("#bigthumb" + id).attr('src').replace("/t/", "/x/");
  });
}
...
function alignTitlesFirstVisit() {
  ...
  var linkHeights = {};
  var maxHeights = {};
  $(".thb_cell:visible").each(function() {
    if (this.id == 'nextbutton') {
      return;
    }
  ...
  for (var top in maxHeights) {
    ...
    for (var i = 0; i < alignedDivs[top].length; i++) {
      ...
      var $div = $("#thb_cell" + id);
      setImportantStyle($div.get(0), {'height': ($div.height() + height) + 'px'});
    }
  }
}
...
function showFlyout(anchor, flyout, keepPrevious, offset) {
  ...
  if (keepPrevious || $(".dt-commentboxfly-lb").not($flyout).length == 0) {
    ...
    $flyout.css({position: 'absolute', zIndex: 11, left: left + 'px', top: top + 'px'}).fadeIn(function() {
      setTimeout(function() {
